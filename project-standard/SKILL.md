...
description: Use when validating or setting up a project's documentation, release flow and git hygiene — checking whether CLAUDE.md/AGENTS.md actually describes the project, whether the code map and product map are current, whether every endpoint is documented, whether releases agree with their tags and changelog, whether local-only files leaked into git, or whether an AI assistant was marked as a contributor. Also use when starting a new repo that should follow the standard from commit 1, when retrofitting an existing repo, or when the user asks "is this project set up right", "check my docs", "scaffold the docs", or "make this repo consistent with the others". This skill reads the repository, never the running product. Do NOT use for judging how an interface looks or behaves in a browser (design-police), or for deciding which open work ships next (next-steps).
version: "0.3.0"
