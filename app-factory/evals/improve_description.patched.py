...
def _generate(
    ...
    """Generate a response. Returns (thinking_text, text).

    Uses Anthropic SDK with extended thinking if ANTHROPIC_API_KEY is set.
    Falls back to claude CLI (no extended thinking) otherwise. The CLI fallback
    means optimizer iterations may converge slightly slower but works without
    a separate API key from console.anthropic.com.
    """
    ...
    # CLI fallback: serialize multi-turn into a single prompt.
    if len(messages) == 1:
    ...
    result = subprocess.run(
        ["claude", "-p", "--model", model, formatted],
        capture_output=True,
        text=True,
        timeout=300,
    )
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    if len(description) > 1024:
        ...
        shorten_thinking, shorten_text = _generate(
            client=client,
            model=model,
            messages=[
